function [C1 P1] = extend_line_segment(A,R,m,c,dAl,C1,P1,C2,mask,Rm,ndvi,eMask,edgeThresh,ndviThresh,entropyThresh,ImgRect, Flag,FlagT,useEntropy)
%A,R,Img,nBin,m,dh,C1,P1,C2,col_hist,peaks,ALS,edgeThresh,htThresh,histThresh
                        ...
                        while (1)                            
                            ...
                            if (ret)
                                ...
                                %plot([P3pa(1,1) P1pa(1,1)], [P3pa(1,2) P1pa(1,2)],'-r'); hold off;
                                %
                                ...
                                if (len1 > len2)
                                    ...
                                    %plot([C1m(1,2) P1m(1,2)], [C1m(1,1) P1m(1,1)],'-g'); hold on;
                                    %plot([P1pam(1,2) P2pam(1,2)], [P1pam(1,1) P2pam(1,1)],'-k'); hold on;
                                    %plot([P2pam(1,2) P4pam(1,2)], [P2pam(1,1) P4pam(1,1)],'-r'); hold on;
                                    %plot([P3pam(1,2) P4pam(1,2)], [P3pam(1,1) P4pam(1,1)],'-k'); hold on;
                                    %plot([P3pam(1,2) P1pam(1,2)], [P3pam(1,1) P1pam(1,1)],'-b'); hold on;
                                    ...
                                    mirP3pam = obj2obs(mirP3pa,Rm);
                                    
                                    %M_oth = find_ALS(A,R,M,mirP3pa,P1pa,C1,mirP1);                                
                                    [totalaP_oth numValaP_oth] = findImagePointCounts(mask, mirP3pam,P1pam,C1m,mirP1m, 0);
                                    ...
                                    mirP4pam = obj2obs(mirP4pa,Rm);
                                    
                                    %M_oth = find_ALS(A,R,M,mirP1,C1,P2pa,mirP4pa);
                                    [totalaP_oth numValaP_oth] = findImagePointCounts(mask, mirP1m,C1m,P2pam,mirP4pam, 0);
                                end
                                nPoints = max([totalaP totalaP_oth]); %max(size(M_left,1), size(M_oth,1));
                                %if (size(ALS_left,1) > 0)
                                     %ALSinfo = sum(ALS_left(:,3) >= htThresh)/size(ALS_left,1);
                                     %MInfo = sum((M_left(:,3) == 0))/nPoints;
                                     MInfo = numValaP/nPoints;%sum((M_left(:,3) == 0))/nPoints;
                                     MoInfo = numValaP_oth/nPoints;%sum((M_oth(:,3) == 0))/nPoints;
                                     if (Flag == 1)
