function lgraph = helperDeeplabv3PlusResnet18(imageSize, numClasses)
...
lgraph = addDecoderToNetwork(lgraph,numClasses,skipLayer);

% Add the pixel classification layers.
lgraph = addPixelClassificationLayers(lgraph);

%--------------------------------------------------------------------------
function lgraph = loadPretrainedResNet18()
% Load ResNet-18 and return as LayerGraph.
net = resnet18;
lgraph = layerGraph(net);

%--------------------------------------------------------------------------
function lgraph = removeLayersAfterFeatureExtractionLayer(lgraph)
...
%--------------------------------------------------------------------------
function lgraph = updateImageInputLayerSize(lgraph,imageSize)
...
%--------------------------------------------------------------------------
function lgraph = reduceNetworkDownSampling(lgraph)
...
%--------------------------------------------------------------------------
function lgraph = increaseConvolutionDilationFactors(lgraph)
...
%--------------------------------------------------------------------------
function lgraph = addPixelClassificationLayers(lgraph)
% Add softmax and pixelClassificationLayers to classify each pixel.
...
%--------------------------------------------------------------------------
function lgraph = addDecoderToNetwork(lgraph,numClasses, lowLevelFeatureExtractor)
